...
class CondRefineNetDilated_small(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.logit_transform = config.data.logit_transform
        # self.norm = ConditionalInstanceNorm2d
...
class CondRefineNetDilated_shallow(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.logit_transform = config.data.logit_transform
        # self.norm = ConditionalInstanceNorm2d
        ...
        self.refine1 = CondRefineBlock([2 * self.ngf], 2 * self.ngf, self.num_classes, self.norm, act=act, start=True)
        self.refine2 = CondRefineBlock([2 * self.ngf, 2 * self.ngf], self.ngf, self.num_classes, self.norm, act=act)
        self.refine3 = CondRefineBlock([self.ngf, self.ngf], self.ngf, self.num_classes, self.norm, act=act, end=True)
    ...
    def forward(self, x, y):
        ...
        ref1 = self.refine1([layer3], y, layer3.shape[2:])
        ref2 = self.refine2([layer2, ref1], y, layer2.shape[2:])
        output = self.refine3([layer1, ref2], y, layer1.shape[2:])
        #output = self.refine4([layer1, ref3], y, layer1.shape[2:])

        output = self.normalizer(output, y)
        output = self.act(output)
        output = self.end_conv(output)
        # output = self.realend(output)
        return output