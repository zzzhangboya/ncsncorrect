...
from models.cond_refinenet_dilated import CondRefineNetDilated, CondRefineNetDilated_small, CondRefineNetDilated_shallow
from torchvision.utils import save_image, make_grid
from PIL import Image
import shutil
from losses.stein import keep_grad, approx_jacobian_trace, exact_jacobian_trace, stein_stats, stein_stats_withscore
from models.MLP import *

__all__ = ['ScoreSteincorrectorRunnner']


class ScoreSteincorrectorRunnner():
    ...
    def train(self):
        ...
        states = torch.load(os.path.join(self.args.run, 'logs', self.args.datasets, 'checkpoint.pth'), map_location=self.config.device)
        ...
        if self.config.data.dataset == 'CIFAR10':
            ...
            dataset = CIFAR10(os.path.join(self.args.run, 'datasets'), train=True, download=False,
                              transform=tran_transform)
            # test_dataset = CIFAR10(os.path.join(self.args.run, 'datasets', 'cifar10_test'), train=False, download=True,
            #                        transform=test_transform)
            test_dataset = CIFAR10(os.path.join(self.args.run, 'datasets'), train=False, download=False,
                                   transform=test_transform)
            ...
            if self.config.data.random_flip:
                dataset = CelebA(root=os.path.join(self.args.run, 'datasets'), split='train',
            ...
            test_dataset = CelebA(root=os.path.join(self.args.run, 'datasets'), split='test',
                                  transform=transforms.Compose([
                                      transforms.CenterCrop(140),
                                      transforms.Resize(self.config.data.image_size),
                                      transforms.ToTensor(),
        ...
        tb_path = os.path.join(self.args.run, 'tensorboard', self.args.datasets, self.args.doc)
        if os.path.exists(tb_path):
            shutil.rmtree(tb_path)

        tb_logger = tensorboardX.SummaryWriter(log_dir=tb_path)

        if self.config.critic.model == "base":
            score = CondRefineNetDilated(self.config).to(self.config.device)
        elif self.config.critic.model == "shallow":
            score = CondRefineNetDilated_shallow(self.config).to(self.config.device)
        elif self.config.critic.model == "small":
            score = CondRefineNetDilated_small(self.config).to(self.config.device)
        ...
        for epoch in range(self.config.training.n_epochs):
            for i, (X, y) in enumerate(dataloader):
                ...
                thisscore = basescore(X, labels).detach()

                X.requires_grad_()
                # stats, norms = stein_stats_withscore(thisscore, X, labels.long(), score, approx_jcb=False, n_samples=1)
                stats, norms = stein_stats_withscore(thisscore, X, labels, score, approx_jcb=True, n_samples=1)
                # stats, norms = stein_stats_withscore(thisscore, X, labels.long(), score, approx_jcb=True, n_samples=8)
                ...
                if step % self.config.training.snapshot_freq == 0:
                    ...
                    difference_norm = self.test(basescore=basescore,score=score,iters=step)
                    tb_logger.add_scalar('score_difference_mean', np.mean(difference_norm), global_step=step) 
                    logging.info("step: {}, score_difference_mean: {}".format(step, np.mean(difference_norm)))
        ...
        with torch.no_grad():
            for c, sigma in tqdm.tqdm(enumerate(sigmas), total=len(sigmas), desc='annealed Langevin dynamics sampling'):
                ...
                for s in range(n_steps_each):
                    ...
                    difference = scorenet(x_mod, labels)
                    difference_norm.append(difference.abs().mean().to('cpu'))
                    print("noise_level: {}, step: {}, difference_norm_mean: {}, difference_norm_max: {}".format(c, s, difference.abs().mean(),
                                                                              difference.abs().max()))
                    grad = basescore(x_mod, labels) - difference
                    x_mod = x_mod + step_size * grad + noise
                    # print("class: {}, step_size: {}, mean {}, max {}".format(c, step_size, grad.abs().mean(),
                    #                                                          grad.abs().max()))

            return images, difference_norm
    ...
    def test(self,basescore=None,score=None,iters=None):
        ...
        if self.config.data.dataset == 'MNIST':
            ...
            all_samples, difference_norm = self.anneal_Langevin_dynamics(samples, basescore, score, sigmas, 100, 0.00002)
        ...
        imgs[0].save(os.path.join(self.args.log, "movie.gif"), save_all=True, append_images=imgs[1:], duration=1, loop=0)
        if iters:
            imgs[0].save(os.path.join(self.args.log, "{}_movie.gif".format(iters)), save_all=True, append_images=imgs[1:], duration=1, loop=0)
            # try:
